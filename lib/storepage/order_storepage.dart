...
class _OrderStorepageState extends State<OrderStorepage> {
  ...
  Stream<List<Map<String, dynamic>>> fetchOrderNotifications() {
    ...
    return FirebaseFirestore.instance
        ...
        .where('username', isEqualTo: currentRestaurantUsername) // ตรวจสอบให้แน่ใจว่าค่า username ถูกต้อง
        .snapshots()
        .map((snapshot) {
      return snapshot.docs.map((doc) {
        final data = doc.data();
        final items = List<Map<String, dynamic>>.from(data['items']); // ดึงรายการ items ที่เป็นอาร์เรย์
        ...
        return {
          ...
          'userId': data['userId'],
          'items': items, // ตรวจสอบให้แน่ใจว่าเราดึงข้อมูลจากอาร์เรย์ items อย่างถูกต้อง
        };
      }).toList();
    });
  }
          ...
          builder: (context, snapshot) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final items = notification['items'] as List<dynamic>; // ดึง items เป็น List<dynamic>

                // ตรวจสอบว่า items ไม่ว่าง
                if (items.isNotEmpty) {
                      ...
                      child: ListTile(
                        leading: ClipRRect(
                          ...
                          child: items[0]['imageUrl'] != null && items[0]['imageUrl'].isNotEmpty
                              ? Image.network(
                                  items[0]['imageUrl'], // ดึงรูปภาพจากรายการ items
                                  width: 60,
                                  height: 60,
                                  fit: BoxFit.cover,
                                )
